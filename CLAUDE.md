...
1. **No runtime reflection.** Decorators are no-ops at runtime; all semantics live in the Rust toolchain. `@anvil-di/strip-decorators` now *depends* on this: it deletes anvil/bellows decorators from source rather than emitting them, so giving any decorator real runtime behaviour would silently break every bundled consumer. A decorator that must do something at runtime needs a different mechanism, not a non-identity stub.
...
| `packages/anvil-unplugin` | `@anvil-di/anvil-unplugin` | Bundler plugin (vite/rollup/webpack/rspack/esbuild). Runs `anvil build` on `buildStart`; watch mode debounces file changes. `mode: "wasm"` runs codegen in-process via the WASM package. Also strips no-op decorators via `@anvil-di/strip-decorators` (`stripDecorators`, default on). 5 tests, all green. |
| `packages/strip-decorators` | `@anvil-di/strip-decorators` | Deletes anvil/bellows decorators from source before bundling, so oxc never has to transform a standard decorator. Binding-based (not name-based) resolution; anything not from a known no-op module is left alone. unplugin entry points for rolldown/rollup/vite/webpack/rspack/esbuild. 21 tests. |
