...
//! tempdir, runs `anvil build --entry <component>.ts`, and diffs the
//! produced `<component>.anvil.ts` against `expected/<component>.anvil.ts`.
//!
...
fn run_fixture(case: &str, entry_file: &str) {
    ...
    let out_name = entry_file.trim_end_matches(".ts").to_owned() + ".anvil.ts";
