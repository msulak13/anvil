...
//!    downstream tooling (e.g. `anvil explain`) can introspect partial
//!    state.
//!
//! Rendering the diagnostics for human display is `anvil-cli`'s job; this
...
///
/// The caller â€” typically `anvil-cli` â€” is responsible for collecting these
...
/// dependencies. The structure is exposed so that `anvil-codegen` and the
/// `anvil explain` debug subcommand can walk it without re-deriving it.
#[derive(Clone, Debug)]
