...
//! cases anvil actually exercises in v0.2:
//!
...
//!   pattern.
//! - **Bare specifiers** (`anvil`, `express`, `@scope/pkg`) — looked up
...
//!   import path-side and passing anvil an absolute-path-keyed map.
//!
...
pub enum MapResolveError {
    ...
    /// The specifier was bare (e.g. `anvil`, `express`) and no
...
/// Try the candidate path verbatim, then with each anvil-supported
