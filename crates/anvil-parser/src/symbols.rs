...
fn rewrite_binding(
    ...
    match &mut b.provider {
        anvil_core::ir::Provider::InjectCtor { class }
        | anvil_core::ir::Provider::ProvidesMethod { module: class, .. } => {
            rewrite_classref(class, resolve_one, discovered)?;
        }
        anvil_core::ir::Provider::Binds { target } => {
            rewrite_key(target, resolve_one, discovered)?;
        }
        anvil_core::ir::Provider::SetMultibinding { contributors } => {
        ...
        anvil_core::ir::Provider::FactoryParam { .. } => {
