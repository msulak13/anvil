//! Configuration loading for the `tsdi` CLI.
//!
//! Two storage formats are supported:
//!
//! 1. A standalone `tsdi.config.json` file.
//! 2. A `tsdi` field on `package.json`.
//!
...
/// Resolved configuration for a `tsdi` invocation.
///
...
pub struct Config {
    ...
    /// emitted file name. Defaults to `.tsdi.ts`. Honored in M5+.
    #[allow(dead_code)]
...
/// Raw JSON shape of a `tsdi` config block. Defaults are applied in
/// [`Config::from_raw`].
...
/// Wrapper for `package.json` so we can pluck out the optional `tsdi` field.
#[derive(Debug, Deserialize)]
struct PackageJson {
    #[serde(default)]
    tsdi: Option<RawConfig>,
}

impl Config {
    /// Load a config from a file. The file may be either a
    /// `tsdi.config.json` (raw config schema) or a `package.json`
    /// (read its `tsdi` field).
    ///
    /// # Errors
    /// Returns an error if the file does not exist, cannot be read,
    /// fails JSON parsing, or — for `package.json` — has no `tsdi`
    /// field.
    pub fn load(path: &Path) -> anyhow::Result<Self> {
            ...
            .is_some_and(|n| n == "package.json")
        {
            ...
            pkg.tsdi
                .ok_or_else(|| anyhow::anyhow!("{} has no `tsdi` field", path.display()))?
        } else {
            serde_json::from_str(&text).map_err(|e| {
                anyhow::anyhow!("failed to parse {} as tsdi config: {e}", path.display())
            })?
        };
    ...
    /// `tsdi.config.json` first, then `package.json` (only if it has a
    /// `tsdi` field). Returns `Ok(None)` if neither matched.
    ///
    /// # Errors
    /// Surfaces I/O or parse errors from the matched candidate.
    pub fn discover(start: &Path) -> anyhow::Result<Option<Self>> {
        let cfg = start.join("tsdi.config.json");
        if cfg.is_file() {
            return Ok(Some(Self::load(&cfg)?));
        }
        let pkg = start.join("package.json");
        if pkg.is_file() {
            // Probe to see whether `tsdi` is present without surfacing
    ...
    fn from_raw(raw: RawConfig, dir: &Path, source: &Path) -> anyhow::Result<Self> {
        ...
        let output_suffix = raw.output_suffix.unwrap_or_else(|| ".tsdi.ts".to_owned());
        let root_dir = raw
            .root_dir
            .map(|r| dir.join(r))
...
mod tests {
    ...
    fn loads_standalone_config_file() {
        ...
        let cfg = Config::load(&root.join("tsdi.config.json")).unwrap();
        let entries = cfg.expand_entries().unwrap();
        assert_eq!(entries.len(), 1);
        assert!(entries[0].ends_with("coffee.ts"));
        assert_eq!(cfg.output_suffix, ".tsdi.ts");
    }

    #[test]
    fn loads_from_package_json_tsdi_field() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path();
        write(&root.join("src/comp.ts"), "// component");
        write(
            &root.join("package.json"),
            r#"{ "name": "x", "tsdi": { "entries": ["src/*.ts"] } }"#,
        );
    ...
    #[test]
    fn discover_prefers_tsdi_config_over_package_json() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path();
        write(&root.join("src/comp.ts"), "// component");
        write(
    ...
    fn empty_entries_list_is_rejected() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path();
        write(&root.join("tsdi.config.json"), r#"{ "entries": [] }"#);
        let err = Config::load(&root.join("tsdi.config.json")).unwrap_err();
