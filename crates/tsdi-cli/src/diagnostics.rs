//! Render `tsdi-core` [`Diagnostic`] values as `miette::Report`s.
//!
//! `tsdi-core` deliberately stays I/O-free, so it ships only structured
...
use tsdi_core::ir::SourceSpan;
use tsdi_core::validate::{Diagnostic, Label};
...
mod tests {
    use super::*;
    use tsdi_core::ir::{Key, ModulePath};
    use tsdi_core::validate::DiagnosticKind;
