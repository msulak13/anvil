...
//! setting the `TSDI_WATCH_ITERATIONS` env var to a positive integer.
...
pub(crate) fn run(plan: &Plan) -> notify::Result<()> {
    ...
    eprintln!("tsdi: watching {}", plan.watch_root.display());
    ...
    for entry in &plan.entries {
        match rebuild_one(entry, plan.tsconfig.clone()) {
            ...
            Err(msg) => {
                eprintln!("tsdi: initial build of {} failed: {msg}", entry.display());
    ...
    let max_iters = std::env::var("TSDI_WATCH_ITERATIONS")
        .ok()
    ...
    loop {
        if let Some(max) = max_iters {
            if iter >= max {
                eprintln!("tsdi: TSDI_WATCH_ITERATIONS={max} reached; exiting");
                return Ok(());
            }
        }
        ...
        for entry in &to_rebuild {
            match rebuild_one(entry, plan.tsconfig.clone()) {
                ...
                Err(msg) => {
                    eprintln!("tsdi: rebuild of {} failed: {msg}", entry.display());
                }
            }
        }
...
fn rebuild_one(entry: &Path, tsconfig: Option<PathBuf>) -> Result<HashSet<PathBuf>, String> {
    ...
    eprintln!(
        "tsdi: rebuilt {} ({} component(s)) in {}ms",
        entry.display(),
        written,
        elapsed.as_millis(),
    );
