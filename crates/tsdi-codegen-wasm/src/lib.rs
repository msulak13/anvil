//! WASM bindings for the tsdi codegen pipeline (M13).
//!
...
//! `*.tsdi.ts` strings plus any structured diagnostics. No
...
//!
//! 1. `tsdi_parser::ProjectGraph::build_from_map` walks the entry's
//!    transitive imports against the supplied [`FileMap`].
//! 2. For each `@Component` in the graph, `tsdi_codegen::emit_component`
//!    produces the `.tsdi.ts` string (parser-validated through
...
use tsdi_core::graph::{build_and_validate, GraphInput};
use tsdi_core::ir::{Binding, ComponentDecl, ModuleDecl, ParsedFile, SubcomponentDecl};
use tsdi_core::validate::Diagnostic;
use tsdi_parser::map_source::{FileMap, MapResolver, PathAlias};
use tsdi_parser::symbols::{ProjectGraph, SymbolError};
...
pub struct CompileOutput {
    /// `*.tsdi.ts` files emitted, one per `@Component`. The host
...
/// JS-friendly view of [`tsdi_core::validate::Diagnostic`].
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct DiagnosticDto {
    /// Stable diagnostic code (e.g. `"tsdi::missing_binding"`).
    pub code: String,
...
/// JS-friendly view of [`tsdi_core::validate::Label`].
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// another Rust crate that wants to embed tsdi without spawning a
...
pub fn compile_native(input: CompileInput) -> Result<CompileOutput, CompileError> {
    ...
    for component in &all_components {
        ...
        if !diags.is_empty() {
            ...
            // only get a `.tsdi.ts` for the components that
            // validated cleanly.
            continue;
        }
        match tsdi_codegen::emit_component(
            component,
            &all_modules,
            &all_inject_classes,
            &all_subcomponents,
            ...
            Ok(contents) => {
                ...
                // `<stem>.tsdi.ts`. Append directly.
                let mut final_path = path_buf.as_os_str().to_owned();
                final_path.push(".tsdi.ts");
            ...
            Err(tsdi_codegen::EmitError::Invalid(diags)) => {
                for d in diags {
                    diagnostics.push(diagnostic_to_dto(&d));
                }
            }
...
fn synthesize_emit_diagnostic(
    component_path: &str,
    err: &tsdi_codegen::EmitError,
) -> DiagnosticDto {
    DiagnosticDto {
        code: "tsdi::emit_failed".to_owned(),
...
mod tests {
    ...
    fn coffee_project() -> CompileInput {
        ...
        files.insert(
            "/proj/node_modules/tsdi/index.d.ts".to_owned(),
    ...
    fn compiles_a_simple_project_to_a_dagger() {
        ...
        assert!(dagger.path.ends_with("app-component.tsdi.ts"));
    ...
    fn compiles_a_windows_absolute_path_project() {
        ...
        let stub = "export const Inject: any; export const Component: any; \
                    export const Singleton: any;";
        files.insert(
            r"C:\proj\node_modules\tsdi\index.d.ts".to_owned(),
            stub.to_owned(),
        );
        files.insert(
            r"C:\proj\src\heater.ts".to_owned(),
            r#"import { Inject } from "tsdi"; @Inject export class Heater {}"#.to_owned(),
        );
        files.insert(
    ...
    fn surfaces_validation_diagnostics_through_the_dto() {
        ...
        assert!(
            out.diagnostics
                .iter()
                .any(|d| d.code == "tsdi::missing_binding"),
