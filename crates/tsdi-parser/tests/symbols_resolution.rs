...
use tsdi_core::ir::{Key, ModulePath, ParsedFile};
...
fn relative_imports_are_resolved_to_absolute_paths() {
    ...
    // Stub `tsdi` package so the resolver can find decorator imports.
    ...
    let root = write_project(
        ...
        &[
            ...
            (
                "src/heater.ts",
                r#"
                    import { Singleton, Inject } from "tsdi";
                    @Inject
                    @Singleton
...
/// Write a minimal `node_modules/tsdi/index.ts` so resolver lookups for
