...
Spec for the TS files emitted by `anvil-codegen`. Update this page whenever the generated shape changes — and bump the version in the banner so users know which compiler produced their files.
...
```ts
// AUTO-GENERATED by anvil v<VERSION>. Do not edit.
// Source: <relative path to component file>
```

The banner is built by [`anvil_codegen::banner_for`](../crates/anvil-codegen/src/lib.rs). The version pin lets users diagnose mismatches between their generated output and a newer toolchain.
...
`@Binds` methods must be `static`. TC39 Stage-3 decorators cannot decorate abstract methods (TS error 1249), so the user writes a static method with a trivial body (`return impl;`). The body still has to compile, but `anvil-codegen` ignores it and emits the delegate above.
...
For each such factory, the emitter appends a second class to the same `.anvil.ts` file:
...
### Emitted `src/coffee/coffee-component.anvil.ts`
```ts
// AUTO-GENERATED by anvil v0.1.0. Do not edit.
...
1. **`anvil_core::graph::build_and_validate`** — refuses to emit if any diagnostic surfaces.
