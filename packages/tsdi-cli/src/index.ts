/**
 * `tsdi-cli` — npm launcher for the native `tsdi` Rust binary.
 *
 * The package itself ships **no binary**. At install time, npm
 * resolves exactly one of the per-platform `optionalDependencies`
 * (e.g. `tsdi-cli-linux-x64` or `tsdi-cli-win32-x64`) based on the
 * `os`/`cpu` fields each declares; only that one is fetched. At
 * runtime, `resolveBinaryPath()` dynamically requires the matching
 ...
 * thing (`npm install tsdi-cli`) and gets the right native binary
...
const BIN_NAME = process.platform === "win32" ? "tsdi.exe" : "tsdi";
...
export function platformPackageName(): string | null {
  ...
  return `tsdi-cli-${suffix}`;
}

/**
 * Locate the native `tsdi` binary on disk.
 *
 * Resolution order:
 *
 * 1. `TSDI_CLI_BIN` env var — handy for tests, monorepo dev, and
...
export function resolveBinaryPath(): string | null {
  // 1. Explicit override.
  const override = process.env.TSDI_CLI_BIN;
...
export function unresolvableBinaryError(): string {
  ...
  if (pkg === null) {
    return [
      `tsdi-cli: no prebuilt binary is available for ${platformInfo}.`,
      "",
      ...
      "  git clone https://github.com/msulak13/tsdi && cargo build --release -p tsdi-cli",
      "and either set TSDI_CLI_BIN=/path/to/tsdi or place the binary on $PATH.",
    ].join("\n");
  }
  return [
    `tsdi-cli: couldn't locate the ${BIN_NAME} binary for ${platformInfo}.`,
    "",
    `Expected the npm package "${pkg}" to be installed alongside`,
    "tsdi-cli. If npm skipped optional dependencies, retry the install:",
    "  npm install --include=optional",
    "  pnpm install               # honors optional deps by default",
    "  yarn install               # honors optional deps by default",
    "",
    "Or set TSDI_CLI_BIN to a binary you've built yourself.",
  ].join("\n");
}

/** Where this `tsdi-cli` package itself is installed (informational). */
